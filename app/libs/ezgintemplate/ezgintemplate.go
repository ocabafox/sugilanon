...
func (r Render) Init() Render {
	...
	for _, view := range viewDirs {
		...
		tpls := append([]string{layout, appLayout, view}, partials...)
...
func (r Render) AddFromFiles(name string, files ...string) *template.Template {
	if strings.Contains(name, "app/") {
		tmpl := template.Must(template.New(filepath.Base(r.AppLayout + r.Ext)).Funcs(r.TemplateFuncMap).ParseFiles(files...))
		r.Add(name, tmpl)
		return tmpl
